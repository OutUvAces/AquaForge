"""
AquaForge runtime implementation (predictor, tiled decode, NMS, spot overlay).

**Do not import this module for application detection.** Use
:mod:`aquaforge.unified.inference` — it exposes only
:func:`~aquaforge.unified.inference.run_aquaforge_tiled_scene_triples` and
:func:`~aquaforge.unified.inference.run_aquaforge_spot_decode`.

``model_manager`` and tests import builders and helpers from here; there is no second detector.
"""
