...
Set ``AQUAFORGE_NO_NE_OCEAN=1`` (or legacy ``VESSEL_DETECTION_NO_NE_OCEAN=1``) to disable
and use SCL-only masking.
"""
...
def _ne_ocean_enabled() -> bool:
    if os.environ.get("AQUAFORGE_NO_NE_OCEAN", "").strip() == "1":
        return False
    # Legacy: any non-empty value disables NE ocean masking.
    if os.environ.get("VESSEL_DETECTION_NO_NE_OCEAN", "").strip() != "":
        return False
    return True
