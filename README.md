...
- **Dynamic balancing** — `DynamicLossBalancer` tracks EMAs of detached per-task losses **within each epoch** and rescales that epoch’s curriculum weights so one task does not drown the others (custom stabiliser; not copied from published GradNorm/DWA implementations).
