"""
Vessel detection — web UI (Streamlit).

**Easiest (no terminal):** double-click ``Open Web App (no terminal).vbs``,
then use the bookmark file ``Vessel detection.url`` or http://127.0.0.1:8501
...
def _streamlit_refresh_review_card_export() -> None:
    ...
    cached — edits to ``vessel_detection/review_card_export.py`` would otherwise be ignored
    ...
    """
    name = "vessel_detection.review_card_export"
    if name not in sys.modules:
        return
    importlib.reload(sys.modules[name])
    rce = sys.modules[name]
    wu = sys.modules.get("vessel_detection.web_ui")
